...
class DialogComebackAll extends React.Component {
    state = {
        data: [],
        ids: [],
        loader: false,
    ...
    getOrders = async () => {
        ...
        let url = `printers/cartridgederefillview/?limit=${this.state.limit}&offset=${(this.state.page - 1) * this.state.limit}`
    ...
    _refillClick= async () =>{
        await this.setState({loader:true})
        let token=localStorage.getItem('token')
        let url=`printers/comebackall/`
        await patchRequest(url,{ids:this.state.ids},{resolve:this.getOrders,reject:()=>{this.setState({loader:false})}},token)
    }
    render() {
        ...
        if (this.state.loader) { return (<React.Fragment><Box height="100vh" display="flex" justifyContent="center" alignItems="center"><CircularProgress /></Box></React.Fragment>) } else {
            return (
                <Box p={5} display="flex" flexDirection="column">
                    ...
                    <Button onClick={this._refillClick}>Принять из заправки</Button>
...
export default DialogComebackAll;