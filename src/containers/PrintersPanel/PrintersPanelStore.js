...
class PrinterPanelStore {
    ...
    getPrinters=async()=>{
        ...
        await getRequest(url,{resolve:(data)=>{this.data=data['results']},reject:this.errorCallback},token)
        this.loader=false
    }
    getInfo=async()=>{
        this.loader=true
    ...
    sendInfo=async()=>{
        ...
        let token=localStorage.getItem('token')
        if(this.selectedAction===1&&this.cabinet===""){this.errorOpen=true;this.errorText="Выберите кабинет";this.loader=false;return;}
